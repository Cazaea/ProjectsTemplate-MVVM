package com.hxd.root.http.rxhttp.utils.httplogger;
...
public final class LoggerInterceptor implements Interceptor {
    ...
    public enum Level {
        ...
        NONE,
        /**
         * Logs request and response lines. * * <p>Example: * <pre>{@code * --> POST /greeting http/1.1 (3-byte body) * * <-- 200 OK (22ms, 6-byte body) * }</pre>
         */
        BASIC,
        /**
         * Logs request and response lines and their respective headers. * * <p>Example: * <pre>{@code * --> POST /greeting http/1.1 * Host: example.com * Content-Type: plain/text * Content-Length: 3 * --> END POST * * <-- 200 OK (22ms) * Content-Type: plain/text * Content-Length: 6 * <-- END HTTP * }</pre>
         */
        HEADERS,
        /**
         * Logs request and response lines and their respective headers and bodies (if present). * * <p>Example: * <pre>{@code * --> POST /greeting http/1.1 * Host: example.com * Content-Type: plain/text * Content-Length: 3 * * Hi? * --> END POST * * <-- 200 OK (22ms) * Content-Type: plain/text * Content-Length: 6 * * Hello! * <-- END HTTP * }</pre>
         */
        BODY
    }
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        String requestStartMessage = "--> "
                + request.method() + ' '
                + RequestUrlUtil.getPageInfo(request.url().toString()) + ' '
                + protocol;
        ...
        if (logHeaders) {
            ...
            if (!logBody || !hasRequestBody) {
                ...
                if (isPlaintext(buffer)) {
                    // 参数格式化
                    String requestParams = ("{" + buffer.readString(Objects.requireNonNull(charset)) + "}");
                    if (requestParams.contains("&"))
                        requestParams = requestParams.replace("&", ",");
                    else if (requestParams.contains("?"))
                        logger.log("===============Hello=============");
                    // 打印动态参数
        ...
        if (logHeaders) {
            ...
            if (!logBody || !HttpHeaders.hasBody(response)) {
                ...
                if (contentLength != 0) {
                    logger.log("");
                    logger.log(buffer.clone().readString(Objects.requireNonNull(charset)).trim());
                }
